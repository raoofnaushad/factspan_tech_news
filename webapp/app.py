...
@app.route('/')
def home():
    newses = get_data_from_mongo(str(today))
    flag_date  = 0
    return render_template('index_new.html', newses = newses, flag_date = flag_date)


@app.route('/', methods=['POST'])
def home_date_selected():
    post_on_date = []
    user_selected_date = datetime.strptime(request.form['date'],"%Y-%m-%d").strftime("%Y-%m-%d")
...
@app.route('/blog/<article>', methods=['POST'])
def blog(article):
    news = get_full_from_mongo(article)
    flag_date  = 0
    return render_template('blog.html', news = news, flag_date = flag_date)
